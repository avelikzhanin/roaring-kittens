...
class ThesisRecord:
    ...
    owner_id: int | None = None  # None только у до-миграционных строк
...
async def save_thesis(session: AsyncSession, *, ticker: str, figi: str, thesis: str,
                      ...
                      entry_price: Decimal | None,
                      backed_by_position: bool = False,
                      owner_id: int) -> ThesisRecord:
    """Один активный тезис на тикер НА ВЛАДЕЛЬЦА: прежний закрывается как superseded."""
    await session.execute(
        update(theses)
        .where(theses.c.ticker == ticker, theses.c.status == "active",
               theses.c.owner_id == owner_id)
...
async def get_active_thesis(session: AsyncSession, ticker: str, *,
                            owner_id: int | None = None) -> ThesisRecord | None:
    stmt = select(theses).where(theses.c.ticker == ticker,
                                theses.c.status == "active")
    if owner_id is not None:
        stmt = stmt.where(theses.c.owner_id == owner_id)
    row = (await session.execute(stmt.limit(1))).first()
    return _row(row) if row else None


async def get_active_theses(session: AsyncSession, *,
                            owner_id: int | None = None) -> list[ThesisRecord]:
    stmt = select(theses).where(theses.c.status == "active")
    if owner_id is not None:
        stmt = stmt.where(theses.c.owner_id == owner_id)
    rows = (await session.execute(stmt.order_by(theses.c.opened_at))).fetchall()
...
async def close_thesis(session: AsyncSession, thesis_id: UUIDType, *, status: str,
    ...
    stmt = update(theses).where(theses.c.id == thesis_id)
    if owner_id is not None:  # гейт колбэков: чужой thesis_id не закроется
        stmt = stmt.where(theses.c.owner_id == owner_id)
    await session.execute(
        stmt.values(status=status, closed_at=datetime.now(tz=timezone.utc),
...
async def get_recently_deleted_tickers(session: AsyncSession,
                                       days: int = 30, *,
                                       owner_id: int | None = None) -> set[str]:
    """Тикеры, чей авто-тезис владелец удалил — не регенерировать месяц."""
    since = datetime.now(tz=timezone.utc) - timedelta(days=days)
    stmt = select(theses.c.ticker).where(
        theses.c.close_reason == "удалён владельцем",
        theses.c.closed_at >= since)
    if owner_id is not None:
        stmt = stmt.where(theses.c.owner_id == owner_id)
    rows = (await session.execute(stmt)).fetchall()
