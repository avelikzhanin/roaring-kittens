...
async def cmd_digest(message: Message, deps: Deps) -> None:
    if message.chat.type != "private":
        # портфель в группу не постим; в личке chat.id == user_id, значит
        # portfolio_cache и asked_by внутри дайджеста ключуются корректно
        await message.answer("Дайджест по твоему портфелю — только в личке со мной.")
        return
