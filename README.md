...
**Сейчас (Phase 5):** сделки — идеи от комитета с уровнями и risk-based размером,
авто-детект покупок/продаж по read-only счёту, сигналы 🎯 цель / 🛑 выход, /deals
с результатами; мультитенантность (инвайты, свои токены, бюджеты), реактивность
(5-мин News Watcher, impact-алерты), память и тезисы, комитет (`/council`),
Trust Loop (`/track`), `/ask`, утренний дайджест.
...
конвертируются в сделки на утренней сверке. /deals — идеи «ждут решения»,
открытые с уровнями, закрытые с результатом. Новости: Интерфакс+РБК — факты,
Смартлаб — только сентимент комитета. Каждая идея: «не является ИИР».

## Команды бота

- `/ask TICKER [вопрос]` — AI-разбор бумаги (напр. `/ask SBER стоит докупать?`) — **доступно всем** (гости: 10/день)
- `/track` — послужной список бота: hit-rate vs IMOEX — **доступно всем**
- `/portfolio` `/digest` `/thesis` `/watch` `/council` `/budget` — **подключённым** (инвайт + свой токен)
- `/seed_retro` — ретро-посев track-record по позициям (60/30 дней назад)
- `/admin invite|users|revoke|set_budget` — **только admin** (владелец)
- `/start` — меню; **первый нажавший /start становится владельцем** (admin)

## Мультитенантность (Phase 4b)

Друг получает инвайт-код (`/admin invite`, 16 hex, TTL 7 дней, ссылка
`t.me/<bot>?start=INV-…`), отправляет его боту, подключает СВОЙ read-only
Tinkoff-токен (шифруется Fernet, сообщение с токеном удаляется; всё только в личке) —
и получает свой портфель/дайджест/тезисы/watchlist/алерты. Расходы каждого юзера
...
Market data общая (системный токен), track-record общий, память комитета скоупится
по юзеру, `/insights` и рефлексия — у admin. Транскрипты и кнопки комитета видит
только инициатор прогона.
