# MOEX Council — Design Spec

> **Статус:** Draft v1 · **Дата:** 2026-06-04 · **Автор:** owner + Claude (brainstorming)
> **Рабочее название:** MOEX Council *(плейсхолдер, можно переименовать)*
...
| Bull/Bear дебаты | reasoning-модель (o-серия) | ~$0.10–0.30 |
| Portfolio Manager | gpt-4o / gpt-4.1 | ~$0.05 |
| Risk Manager | reasoning-модель | вкл. выше |
| Weekly reflection | топовая доступная | ~$0.30–0.50 |

> **Открытый вопрос:** точный список reasoning-моделей, доступных на ключе пользователя (o3/o4-mini и т.п.) — влияет на routing.
...
4. **Импорт существующих позиций:** при onboarding — ретроспективный тезис (AI-реконструкция) или ручной ввод.
...
|---|---|---|---|
| **0. Фундамент** | 1 | Repo, Docker, Postgres+pgvector, Fernet, aiogram skeleton, Tinkoff connect, `/portfolio`, логи, error-каркас | Бот-«зеркало портфеля» |
...
1. **Reasoning-модели на ключе пользователя** — какие именно доступны (o3/o4-mini/...)? Влияет на routing для дебатов и Risk Manager.
2. **Хостинг:** Railway реально подключается к Tinkoff API с US/EU IP? Проверить в Фазе 0; иначе fallback на Timeweb.
3. **Финальное название** проекта (текущее — плейсхолдер MOEX Council).
4. **Dev-окружение:** локальная разработка через VPN на RU-IP или сразу dev на сервере?
5. **Глубина импорта истории** при onboarding: реконструировать тезисы для всех старых позиций или только активных крупных?
```
