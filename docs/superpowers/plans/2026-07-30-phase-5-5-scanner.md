...
6. Терминология (только юзер-тексты, код не трогаем). ГЛОССАРИЙ (ревью: «идея» уже
   занята сделками — «Идея сделки №N», второй смысл запрещён): сделки = «идея»;
   тезисный слой = «причина держать» / «сопровождение». Замены:
   - render.py «🎯 Тезис:» → «🎯 Суть: …»
   - кнопки «📌 Принять тезис / Принять новый тезис» → «📌 Взять на сопровождение»
   - валидатор: «⚠️ Тезис по X ослаблен» → «⚠️ Причина держать X под вопросом»,
     «СЛОМАН» → «🚨 Причина держать X сломана», «\nТезис: …» → «\nПричина была: …»,
     «Новости ломают тезис по X» → «Новости ломают причину держать X»
   - thesis.py: «📌 Активные тезисы:» → «📌 Почему держим:», пустое состояние и hint
     без слова «тезис»; « · идея» (unbacked-маркер) → « · ещё не куплено»;
     «📌 Тезис по X принят» → «📌 Взял X на сопровождение»; «Этот тезис уже принят»
     → «Уже на сопровождении»; «🗑 Тезис закрыт.» → «🗑 Убрал, больше не сопровождаю.»;
     «Не нашёл данные комитета для тезиса.» → «…для этой бумаги.»; format_insights
     «закрытые тезисы» → «закрытые сделки»
   - positions_sync: «Сгенерировал тезис:» → «Почему держим:»; «…(вес ≥5%) без
     тезиса» → «…(вес ≥5%) без сопровождения»; кнопка «🗑 Удалить тезис» →
     «🗑 Не согласен — убрать»; «📕 Позиция X закрыта — тезис закрыт{sign}.\nТезис
     был:» → «📕 Позиция X закрыта — сопровождение завершено{sign}.\nПричина была:»;
     «за время тезиса» → «за время позиции»
   - onboarding.py: «Тезисы для позиций ≥5% появятся…» → «Для позиций ≥5% я сам
     объясню, почему их держать, после ближайшей утренней сверки.»
   - Финальная проверка: `grep -rn "[Тт]езис" src/` — в строках юзерских сообщений
     слова нет (докстринги/имена — можно). Команда /thesis остаётся.
...
13 находок → 10 уникальных подтверждено (0 отклонено), внесены: days=90 для MA50;
guard сканера по council_runs (видит вето-прогоны!) с окном 7 дней — новый
`council_run_recently` в db/council.py; перезагрузка universe при пустых весах +
различимые логи; source='scanner' параметром в propose_deal_from_council; глоссарий
«идея=сделка / причина держать=тезис» (коллизия смыслов); полный список замен
терминологии (+5 строк, +onboarding.py); HELP с admin-блоком и /history;
reason_short в лог; guard неторгового дня (последняя свеча < сегодня → skip).
